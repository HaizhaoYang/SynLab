% This code tests the time complexity of 1D (synchrosqueezed) fwd and inv wave packet transform with arbitrary number of samples
%
% By Haizhao Yang
...
%% Set up tests
test = cell(2,1); rslt = cell(2,1);
test{1,1} = struct('num',0,'is_real',0,'is_unif',1,'typeNUFFT',1,'wpttime',[],'sstime',[],'isstime1',[],'isstime2',[]);
%test{2,1} = struct('num',0,'is_real',0,'is_unif',0,'typeNUFFT',2,'wpttime',[],'sstime',[],'isstime1',[],'isstime2',[]);
...
for cnt = 1:1
    figure;plot(st:ed,log2((test{cnt,1}.wpttime)./(st:ed)));
    figure;plot(st:ed,log2((test{cnt,1}.sstime)./(st:ed)));
    figure;plot(st:ed,log2((test{cnt,1}.isstime1)./(st:ed)));
    figure;plot(st:ed,log2((test{cnt,1}.isstime2)./(st:ed)));
end
