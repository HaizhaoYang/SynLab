...
switch example
    ...
    case 3
        % A photograph of a bubble raft, some local wave vectors have very weak energy, not good
        phi = imread('GB3.jpg');
        phi = rgb2gray(phi);
        [m n] = size(phi);
    ...
    case 6
        % three isolated defects with a small angle boundary
        phi = imread('GB8.png');
        phi = double(phi);
        parameters = struct('extendRatio',0.1,'threRatio',0.1, ...
            'epsFreqBand',1e-2,'is_real',0,'numWave',3, ...
...
%% -------------------------------------------------------------------------
% find defect region and their connected components  binaryIndexCoarse = 1-findDeftArea(smoothImage( sum(masses,3), 4, 1),energyThre,coarseThre);
